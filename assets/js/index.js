    ...
    const pokemonHtml = `
        <li class="pokemons-item">
            <div class="pokemons-card" data-pokemon-card-type-name="electric">
                <div class="pokemons-card-image-container js-pokemon-card-image-container">
                    <img height=200 src="${pokemon.sprites.front_default}" alt="${pokemon.name}" class="pokemons-card-image">
                </div>
                ...
                <ul class="pokemons-card-attributes">
                    <li class="pokdemons-card-attributes-item">
...
async function handleSearchInput(event, pokemons) {
    ...
    if (!value) {
        removeAllPokemons()  //limpar os pokemons depois da pesquisa e trazer a lista original sem duplicar o pokemon pesquisado
...
async function main(){
    ...
    console.log("Primeiro Pokemon: ", responseJson.results[0].name)
}
